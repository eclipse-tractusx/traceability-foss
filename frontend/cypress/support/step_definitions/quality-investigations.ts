...
When("open details of created {string}", () => {
  cy.get('[data-testid="table-menu-button"]').first().click(); //the first investigation will be opened
    if (cy.get('[data-testid="table-menu-button--actions.viewDetails"]').should('not.exist')) { //this is necessary because sometimes the page reload and the first click disappear
