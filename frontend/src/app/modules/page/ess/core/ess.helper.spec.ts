...
describe('resetMultiSelectionAutoCompleteComponent', () => {
    it('should reset multiSelectAutocompleteComponents and set oneFilterSet to true if filterFormGroup is dirty', () => {
        ...
        const essTableComponents: EssTableComponent[] = [
            {
                // @ts-ignore
        ...
        const essTableComponentsQueryList = mockQueryList(essTableComponents);

        let oneFilterSet = false;

        // Act
        oneFilterSet = resetMultiSelectionAutoCompleteComponent(essTableComponentsQueryList, oneFilterSet);

        // Assert
        expect(oneFilterSet).toBe(true);

        essTableComponents.forEach((partsTableComponent) => {
