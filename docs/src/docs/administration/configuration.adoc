...
// TODO add values.yaml from frontend repository and describe the values.

== Helm configuration Trace-X Backend (values.yaml)

[source,yaml]
----
include::../../../../charts/product-traceability-foss-backend/values.yaml[lines=121..-1]
----

=== Values explained

==== <springprofile>
The different profiles for the different supported environments to bootstrap the resources which are required for the certain environments.
...
The URL of the Keycloak token API. Used by the Trace-X for token creation to authenticate with other services.

==== <oauth2.jwkSetUri>
The URL of the Keycloak JWK Set. Used by the Trace-X to validate tokens when the Trace-X API is called.

==== <oauth2.resourceClient>
The Client which is used to auth the backend.
...
=== <pgadmin4.enabled>
Enables <true> or disables <false> pgadmin4 console for postgreSQL database instance

=== <pgadmin4.ingress.enabled>
Enables <true> or disables <false> ingress proxy for pgadmin4 console
