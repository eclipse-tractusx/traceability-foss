...
This section describes what happens when the user wants to get a report of the imported assets associated to a jobId.
...
If yes, then the endpoint returns an import report to the given import job id.

If the import job id is not known to Trace-X, an HTTP 404 error is returned.
