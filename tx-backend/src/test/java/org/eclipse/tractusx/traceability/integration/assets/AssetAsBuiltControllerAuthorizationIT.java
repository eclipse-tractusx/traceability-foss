 ...
 * the endpoints of {@link org.eclipse.tractusx.traceability.assets.application.asbuilt.rest.AssetAsBuiltController}.
 */
class AssetAsBuiltControllerAuthorizationIT extends IntegrationTestSpecification {

    private static final String ROOT = "/api/assets/as-built";

    @ParameterizedTest
    @MethodSource("org.eclipse.tractusx.traceability.integration.common.support.RoleSupport#allRolesAllowed")
    void shouldAllowAsBuiltEndpointOnlyForSpecificRoles(JwtRole role, boolean isAllowed) throws JoseException {
        given()
    ...
    @ParameterizedTest
    @MethodSource("org.eclipse.tractusx.traceability.integration.common.support.RoleSupport#allRolesAllowed")
    void shouldAllowCountriesEndpointOnlyForSpecificRoles(JwtRole role, boolean isAllowed) throws JoseException {
        given()
                .header(oAuth2Support.jwtAuthorizationWithOptionalRole(role))
                .contentType(ContentType.JSON)
                .when()
